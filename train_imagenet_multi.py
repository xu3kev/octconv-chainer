...
def main():
    ...
    parser.add_argument('--alpha', type=float, default=0.25, help='octave convolution alpha parameter')
    ...
    extractor = model_cfg['class'](
        n_class=len(label_names), alpha=args.alpha, **model_cfg['kwargs'])
    ...
    for l in model.links():
        if isinstance(l, Bottleneck):
            if l.conv3.bn_h is not None:
                l.conv3.bn_h.gamma.data[:] = 0
