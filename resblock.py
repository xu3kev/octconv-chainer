...
class ResBlock(PickableSequentialChain):
    ...
    def __init__(self, n_layer, in_channels, mid_channels,
                 ...
                 bn_kwargs={}, stride_first=False, add_seblock=False, alpha=0):
        ...
        with self.init_scope():
            self.a = Bottleneck(
                ...
                stride_first=stride_first, add_seblock=add_seblock, alpha=alpha)
...
class Bottleneck(chainer.Chain):
    ...
    def __init__(self, in_channels, mid_channels, out_channels,
                 ...
                 residual_conv=False, stride_first=False, add_seblock=False, alpha=0):
        if stride_first:
            first_stride = stride
            second_stride = 1
        else:
        ...
        with self.init_scope():
            self.conv1 = Conv_BN_ACT(in_channels, mid_channels,
                                       1, first_stride, 0,
                                       nobias=True, initialW=initialW,
                                       bn_kwargs=bn_kwargs, alpha_out=alpha)
            # pad = dilate
            self.conv2 = Conv_BN_ACT(mid_channels, mid_channels,
                                       3, second_stride, dilate, dilate,
                                       groups, nobias=True, initialW=initialW,
                                       bn_kwargs=bn_kwargs, alpha_out=alpha)
            self.conv3 = Conv_BN_ACT(mid_channels, out_channels, 1, 1, 0,
                                       nobias=True, initialW=initialW,
                                       activ=None, bn_kwargs=bn_kwargs, alpha_out=alpha)
            assert add_seblock == False
            if add_seblock:
                self.se = SEBlock(out_channels)
            if residual_conv:
                self.residual_conv = Conv_BN_ACT(
